...
$pt->config->agora_enabled = (isset($pt->config->agora_enabled) ? $pt->config->agora_enabled : 1);
$pt->config->chef_live_enabled = (isset($pt->config->chef_live_enabled) ? $pt->config->chef_live_enabled : 0);
$pt->config->live_provider = (!empty($pt->config->live_provider) ? $pt->config->live_provider : 'agora');
$pt->config->live_external_servers_json = (isset($pt->config->live_external_servers_json) ? $pt->config->live_external_servers_json : '[]');
$pt->config->chef_live_api_url = (!empty($pt->config->chef_live_api_url) ? $pt->config->chef_live_api_url : getenv('CHEF_LIVE_API_URL'));
$pt->config->chef_live_rtmp_url = (!empty($pt->config->chef_live_rtmp_url) ? $pt->config->chef_live_rtmp_url : getenv('CHEF_LIVE_RTMP_URL'));
$pt->config->chef_live_hls_url = (!empty($pt->config->chef_live_hls_url) ? $pt->config->chef_live_hls_url : getenv('CHEF_LIVE_HLS_URL'));
$pt->config->chef_live_webrtc_url = (!empty($pt->config->chef_live_webrtc_url) ? $pt->config->chef_live_webrtc_url : getenv('CHEF_LIVE_WEBRTC_URL'));
$chef_live_defaults = PT_GetChefLiveDefaults();
foreach ($chef_live_defaults as $chef_live_key => $chef_live_default) {
    if (!isset($pt->config->{$chef_live_key}) || $pt->config->{$chef_live_key} === '') {
        $pt->config->{$chef_live_key} = $chef_live_default;
    }
}
$pt->config->active_live_provider = PT_GetDefaultLiveProvider();
