    ...
    if (!empty($submit_data)) {
        ...
        foreach ($submit_data as $key => $value) {
            ...
            if ($key == 'bank_description') {
                $update = PT_SaveConfigValue($key, $value);
            }
            else{
                if ($key != 'fav_category') {
                    $update = PT_SaveConfigValue($key, PT_Secure($value, 0));
                }
            }
...
if ($first == 'chef-live-health') {
    $response = PT_ChefLiveRequest('GET', '/health');
    if (!empty($response['status']) && $response['status'] >= 200 && $response['status'] < 300) {
        $data = array(
            'status' => 200,
            'data' => $response
        );
        ...
        $data = array(
            'status' => 400,
            'message' => (!empty($response['error']) ? $response['error'] : 'Chef Live ne répond pas.'),
            'data' => $response
        );
    }
    header('Content-Type: application/json');
...
if ($first == 'chef-live-config') {
    $response = PT_ChefLiveRequest('GET', '/config');
    if (!empty($response['status']) && $response['status'] >= 200 && $response['status'] < 300) {
        $data = array(
            'status' => 200,
            'config' => (!empty($response['config']) ? $response['config'] : array())
        );
    }
    else{
        $data = array(
            'status' => 400,
            'message' => (!empty($response['error']) ? $response['error'] : 'Impossible de charger la configuration Chef Live.'),
            'data' => $response
        );
    }
    header('Content-Type: application/json');
...
if ($first == 'chef-live-sync') {
    $response = PT_ChefLiveRequest('PUT', '/config', PT_GetChefLiveConfigPayload());
    if (!empty($response['status']) && $response['status'] >= 200 && $response['status'] < 300) {
        $data = array(
            'status' => 200,
            'message' => 'La configuration Chef Live a été synchronisée.',
            'config' => (!empty($response['config']) ? $response['config'] : array())
        );
    }
    else{
        $data = array(
            'status' => 400,
            'message' => (!empty($response['error']) ? $response['error'] : 'Impossible de synchroniser Chef Live.'),
            'data' => $response
        );
    }
    header('Content-Type: application/json');
...
if ($first == 'chef-live-runtime') {
    $response = PT_ChefLiveRequest('GET', '/runtime');
    if (!empty($response['status']) && $response['status'] >= 200 && $response['status'] < 300) {
        $data = array(
            'status' => 200,
            'runtime' => (!empty($response['runtime']) ? $response['runtime'] : array())
        );
    }
    else{
        $data = array(
            'status' => 400,
            'message' => (!empty($response['error']) ? $response['error'] : 'Impossible de charger l’état Chef Live.'),
            'data' => $response
        );
    }
    header('Content-Type: application/json');
...
if ($first == 'tinymce-upload') {
    header('Content-Type: application/json; charset=utf-8');

    if (empty($_FILES['file']) || !empty($_FILES['file']['error']) || !file_exists($_FILES['file']['tmp_name'])) {
        echo json_encode(array(
            'status' => 400,
            'error' => 'Aucun fichier image valide reçu.'
        ));
        exit();
    }

    $image = @getimagesize($_FILES['file']['tmp_name']);
    if (empty($image) || !in_array($image[2], array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG, IMAGETYPE_BMP, IMAGETYPE_WEBP))) {
        echo json_encode(array(
            'status' => 400,
    ...
    if (empty($file_upload['filename'])) {
        echo json_encode(array(
            'status' => 500,
            'error' => !empty($file_upload['error']) ? $file_upload['error'] : 'Impossible de téléverser l’image.'
        ));
        exit();
    }
    ...
    echo json_encode(array(
        'status' => 200,
        'location' => PT_GetMedia($file_upload['filename'])
    ));
    exit();
}
