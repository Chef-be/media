...
$get_video->is_still_live = PT_IsVideoCurrentlyLive($get_video);
$get_video->live_sub_users = 0;
if ($get_video->is_still_live) {
    if (empty($get_video->live_time) || $get_video->live_time < (time() - 10)) {
        $get_video->live_time = time();
        $db->where('id', $get_video->id)->update(T_VIDEOS, array('live_time' => $get_video->live_time));
    }
    $get_video->live_sub_users = PT_GetCurrentLiveAudienceCount($get_video);
}
