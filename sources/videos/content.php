...
if ($page == 'trending') {
    ...
    // pagination system 
    $videos = $db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie',0)->where('live_ended', 0)->where('stream_name', '', '!=')->where('live_time', time() - PT_GetLiveHeartbeatWindow(), '>=')->where('approved',1)->where('is_short', 0)->orderBy('live_time', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
    $pt->total_pages = $db->totalPages;
    // pagination system 
} 
