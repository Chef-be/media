...
if ($pt->config->live_video == 1) {
    if (!empty($pro_users)){
        ...
        $live_data = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('is_short', 0)->where('live_ended', 0)->where('stream_name', '', '!=')->where('live_time', time() - PT_GetLiveHeartbeatWindow(), '>=')->get(T_VIDEOS, 4);
    }

    if (empty($live_data)) {
        $db->where('privacy', 0);
        $live_data = $db->where('is_movie', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('approved',1)->where('is_short', 0)->where('live_ended', 0)->where('stream_name', '', '!=')->where('live_time', time() - PT_GetLiveHeartbeatWindow(), '>=')->orderBy('live_time', 'DESC')->get(T_VIDEOS, $limit);
    }
