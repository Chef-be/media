...
elseif (empty($_POST['type'])) {
	...
	if ($_POST['type'] == 'create') {
		...
		if (empty($target)) {
			$response_data    = array(
			    ...
			    'errors' => array(
		            'error_id' => '5',
		            'error_text' => 'invalid live target'
		        )
			);
			return;
		}
		...
		if (empty($stream_name)) {
	        ...
	        if ($provider == 'chef_live') {
	        	$chef_live = PT_RegisterChefLiveStream($stream_name, $pt->user->id, $post_id, 'live video '.$pt->user->name);
	        	...
	        	$db->where('id', $post_id)->update(T_VIDEOS, array(
	        		'agora_resource_id' => json_encode(array(
	        			'provider' => 'chef_live',
	        			'server_id' => 'chef_live',
	        ...
	        if ($provider == 'agora' && $pt->config->live_video == 1 && !empty($pt->config->agora_app_id) && !empty($pt->config->agora_customer_id) && !empty($pt->config->agora_customer_certificate) && $pt->config->live_video_save == 1) {
	            ...
	            if ($pt->config->amazone_s3_2 == 1 && !empty($pt->config->bucket_name_2) && !empty($pt->config->amazone_s3_key_2) && !empty($pt->config->amazone_s3_s_key_2) && !empty($pt->config->region_2)) {
	                ...
	                if (in_array(strtolower($pt->config->region_2),array_keys($region_array) )) {
	                    ...
	                    StartCloudRecording(1,$region_array[strtolower($pt->config->region_2)],$pt->config->bucket_name_2,$pt->config->amazone_s3_key_2,$pt->config->amazone_s3_s_key_2,$stream_name,explode('_', $stream_name)[2],$post_id);
	                }
	        ...
	        $response_data     = array(
		        ...
		        'provider' => $provider,
		        'server_id' => $target['server_id'],
		        'provider_label' => $target['name']
		    );
		    if (!empty($external)) {
		    	$response_data['publish_url'] = $external['publish_url'];
		    	$response_data['playback_url'] = $external['playback_url'];
		    	$response_data['stream_key'] = $external['stream_key'];
		    	$response_data['webrtc_url'] = $external['webrtc_url'];
		    }
		    if (!empty($browser_publish_url)) {
		    	$response_data['browser_publish_url'] = $browser_publish_url;
		    }
    	}
	}
	...
	elseif ($_POST['type'] == 'delete') {
		if (!empty($_POST['post_id']) && is_numeric($_POST['post_id']) && $_POST['post_id'] > 0) {
			...
			$provider = PT_GetLiveProvider($post);
	        ...
	        if ($pt->config->live_video_save == 0) {
	            ...
	            if ($provider == 'chef_live' && !empty($post)) {
	            	PT_StopChefLiveStream($post->stream_name);
	            }
